...
public class CloudDatastoreRefUtil {
	private static final Logger LOG = LoggerFactory.getLogger(CloudDatastoreRefUtil.class);
	...
	private Object retrieveEntityColumnValue(Entity entity, String dataStoreColumName,
		...
		case STRING:
			if (JavaClassType.STRING_TYPE.equals(dataStoreColumnTypeShouldBe))
				...
				LOG.debug("JsonToListObject : "+dataStoreColumnTypeShouldBe);
			...
			{	
				LOG.debug("JsonToListCustomObject : "+dataStoreColumnTypeShouldBe);
				...
				LOG.debug("JsonStringToObject : "+dataStoreColumnTypeShouldBe);
			...
			{	
				LOG.debug("JsonToMapObject : "+dataStoreColumnTypeShouldBe);
				result = getMapObject(setter, dataStoreValue,dataStoreColumnTypeShouldBe);
			}
			else
				LOG.error("STRING Error : New Type " + dataStoreColumnTypeShouldBe);
			break;
		case LIST:
		...
		case TIMESTAMP:
			...
			break;
		default:
			LOG.debug("IN SWITCH DEFAULT Error : New Type " + dataStoreColumnTypeShouldBe +", "+dataStoreValue.getType());
		}
		return result;
	}
		...
		for (Type genericParameterType : genericParameterTypes) {
			if (genericParameterType instanceof ParameterizedType) {
				...
				for (Type parameterArgType : parameterArgTypes) {
					...
					LOG.debug("parameterArgClass = " + parameterArgClass.getSimpleName() + "--"
	...
	private Object getListCustomObject(Method setter, Value<?> dataStoreValue, String dataStoreColumnTypeShouldBe)
		...
		for(Type genericParameterType : genericParameterTypes){
		    if(genericParameterType instanceof ParameterizedType){
		        ...
		        for(Type parameterArgType : parameterArgTypes){
		            Class parameterArgClass = (Class) parameterArgType;
		            LOG.debug("parameterArgClass = " + parameterArgClass.getSimpleName() +"--"+parameterArgClass.getName());
		            Class<?> cls = Class.forName(parameterArgClass.getName());
					result =gson.fromJson((String)dataStoreValue.get(),  getType(List.class, cls));
		        }
		    }
	...
	public BaseEntity.Builder setModelToEntityBuilder(KeyFactory keyFactory, Object bean, boolean isAddOperation) {
		...
		try {
			...
			for (PropertyDescriptor pd : props) {
				...
				LOG.info(name + " = " + value + ", type = " + type + ", name = " + type.getName());
	...
	private void setGenericBuilderList(BaseEntity.Builder builder, String name, Method getter, Object value) {
		...
		if (returnType instanceof ParameterizedType) {
			...
			for (Type typeArgument : typeArguments) {
				...
				LOG.debug("typeArgClass = " + typeArgClass.getSimpleName() + "--" + typeArgClass.getName());
